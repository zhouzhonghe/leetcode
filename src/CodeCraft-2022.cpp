...
int main() {
    ...
    printf("一共%d个客户节点，%d个边缘节点，%d个时刻, 延迟限制：%d\n", N, M, T, qos_constraint);
    cout << "读取数据完成" << endl;
    ...
    for (int i = 0; i < M; ++i) {
        serverList[i] = new serverNode(8008, 0, serverID[i]);
    ...
    for (int i = 0; i < M; ++i) {
        if (serverList[i]->calltimes != 0) {
            serverList[i]->weight =
                serverList[i]->maxBW * maxWeight / serverList[i]->calltimes / maxValue;
    ...
    for (int i = 0; i < T; ++i) {
        ...
        while (isOutrange || iter < 100) {
            ...
            if (!isOutrange) {
                ...
                avgBW /= num;
                // cout << serverMap[mostName]->bw << " " << serverMap[leastName]->bw << " " <<
                // avgBW
                //      << endl;
                int base = 3;
                for (auto it : serverList) {
    ...
    for (int i = 0; i < T; ++i) {
        for (int j = 0; j < N; ++j) {
            ...
            for (int k = 0; k < res[i][j].size(); ++k) {
                ...
                if (res[i][j][k].second <= 0) {
                    cout << "分配<=0不合法" << endl;
                }
                if (serverMap.count(res[i][j][k].first) == 0) {
                    cout << "不合法" << endl;
                }
            }
            if (tmp != demand[i][j]) {
